...
#if (HALMAC_8822B_SUPPORT && HALMAC_SDIO_SUPPORT)
...
static enum halmac_ret_status
update_ac_empty_8822b(struct halmac_adapter *adapter, u8 value);
...
/**
 * cfg_tx_fmt_sdio_8822b() - config sdio tx address format
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
cfg_tx_fmt_sdio_8822b(struct halmac_adapter *adapter,
...
update_oqt_free_space_8822b(struct halmac_adapter *adapter)
{
	...
	update_ac_empty_8822b(adapter, data[4]);
...
update_sdio_free_page_8822b(struct halmac_adapter *adapter)
{
	...
	update_ac_empty_8822b(adapter, data[12]);
...
update_ac_empty_8822b(struct halmac_adapter *adapter, u8 value)
{
	...
	if (free_space->ac_oqt_num == OQT_ENTRY_AC_8822B) {
		while (value > 0) {
			value = value & (value - 1);
			free_space->ac_empty++;
		};
		...
		PLTFM_MSG_TRACE("[TRACE]free_space->ac_oqt_num %d != %d\n",
				free_space->ac_oqt_num, OQT_ENTRY_AC_8822B);
	}
